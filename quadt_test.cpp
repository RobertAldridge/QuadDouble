...
template<class T> const T& constants<T>::pi = T::_pi;

template<class T> const T& constants<T>::pi2 = T::_pi2;

template<class T> const T& constants<T>::pi4 = T::_pi4;

template<class T> const T& constants<T>::log2 = T::_log2;

template<> const double& constants<double>::pi = 3.14159265358979;

template<> const double& constants<double>::pi2 = 1.57079632679490;

template<> const double& constants<double>::pi4 = 0.785398163397448;

template<> const double& constants<double>::log2 = 0.693147180559945;

template<class T> class CircleFunction
{
  T r;
...
template<class T> class SecantFunction
{
public:
...
template<class T> class TestFunction1
{
public:
...
template<class T> class TestFunction2
{
public:
...
template<class T> class TestFunction3
{
public:
...
template<class T> class TestFunction4
{
public:
...
template<class T> class TestFunction5
{
public:
...
template<class T> void convert(char* s, T* x)
{
  *x = s;
}

template<> void convert(char* s, double* x)
{
  *x = atof(s);
}

template<class T> class CosineProduct
{
  T coeff[32];
...
template<class T> class InvCosineProduct
{
  T coeff[32];
...
template<class T> class quadt_tester
{
  double eps;
...
public:
  ...
  template<class F> void test_integral(F& f, T a, T b, T truth);

  void test();
};

template<class T> template<class F> void quadt_tester<T>::test_integral(F& f, T a, T b, T truth)
{
  int r = 0;
...
template<class T> void quadt_tester<T>::test()
{
  CosineProduct<T> f8;
  ...
  if( !flag_last_only)
  {
    ...
    test_integral(f4, T(0.0), T(1.0), constants<T>::pi4/3.0 - T(1.0) / 6.0 + log(T(2.0) ) / 6.0);
    ...
    test_integral(f6, T(0.0), constants<T>::pi2, constants<T>::pi2 * sqrt(T(2.0) ) );
...
template<class T> void test_quadt(double eps)
{
  tictoc tv;

  memset(&tv, 0, sizeof(tictoc) );
...
static void print_usage()
{
  ...
  cout << "Performs a selected set of integration using the quad-double library." << endl;
  ...
  cout << "--qd Perform quadrature test with quad-double.  This is the default." << endl;

  cout << "--all Perform quadrature test with all three precision types." << endl;

  cout << "-v --verbose Prints out detailed test results." << endl;

  cout << "-x Perform only the last test, an interesting quadrature whose value is *very* close to pi / 8 (see paper)." << endl;
}
